...
function IntegrationsView() {
  ...
  const [connector, setConnector] = useState<"shopify" | "slack">("shopify");
  const [values, setValues] = useState<Record<string, string>>({});
  const [action, setAction] = useState("list_products");
  const [channel, setChannel] = useState("");
  const [text, setText] = useState("");
  const [notice, setNotice] = useState("");
  const [approvalId, setApprovalId] = useState("");
  const connectedSet = new Set(connected.map(item => item.connector));
  const saveConnection = async () => {
    await save.mutateAsync({ connector, values });
    setValues({});
    setNotice(`${connector === "shopify" ? "Shopify" : "Slack"} connection saved securely.`);
  };
  const previewAction = async () => {
    const parameters = connector === "shopify"
      ? action === "update_product_title" ? { productId: values.productId ?? "", title: values.title ?? "" } : { first: 10 }
      : action === "send_message" ? { channel, text } : { limit: 50 };
    const result = await preview.mutateAsync({ connector, action: action as "list_products" | "update_product_title" | "list_channels" | "send_message", parameters });
  ...
  return <div className="page workspace-page"><div className="workspace-heading"><div><p className="eyebrow"><span className="eyebrow-line" /> CONNECTED WORKFLOWS</p><h1>Apps & Integrations</h1><p>Connect Shopify and Slack. Hanna previews every action, waits for approval, then verifies the result.</p></div><div className="secure-badge"><Check size={14} /> Tokens stay server-side</div></div>{!user && <div className="login-note"><Inbox size={17} /><div><strong>Firebase Auth is required for live connectors</strong><p>Connect the server-side Firebase adapter to bind credentials and actions to a user account.</p></div></div>}{notice && <div className="success-note"><Check size={15} /> {notice}</div>}<div className="model-grid"><div className="model-card"><div className="model-logo green"><ShoppingBag size={19} /></div><div className="model-card-copy"><div className="model-card-top"><strong>Shopify</strong><span>{connectedSet.has("shopify") ? "Connected" : "Not connected"}</span></div><h3>Admin GraphQL</h3><p>Read products or update a product title after approval.</p></div><button className="icon-btn" onClick={() => setConnector("shopify")} aria-label="Select Shopify"><ArrowUpRight size={16} /></button></div><div className="model-card"><div className="model-logo blue"><MessageSquare size={19} /></div><div className="model-card-copy"><div className="model-card-top"><strong>Slack</strong><span>{connectedSet.has("slack") ? "Connected" : "Not connected"}</span></div><h3>Web API</h3><p>List accessible channels or post approved messages.</p></div><button className="icon-btn" onClick={() => setConnector("slack")} aria-label="Select Slack"><ArrowUpRight size={16} /></button></div></div><div className="settings-modal" style={{ maxWidth: 720, marginTop: 24 }}><div className="settings-modal-header"><div><span className="section-kicker">{connector.toUpperCase()} CONNECTION</span><h2>{connector === "shopify" ? "Connect Shopify" : "Connect Slack"}</h2></div><span className="connected-status">Backend only</span></div><div className="provider-grid"><label className="key-label">Action<select value={action} onChange={e => setAction(e.target.value)}><option value={connector === "shopify" ? "list_products" : "list_channels"}>{connector === "shopify" ? "List products" : "List channels"}</option><option value={connector === "shopify" ? "update_product_title" : "send_message"}>{connector === "shopify" ? "Update product title" : "Send message"}</option></select></label>{connector === "shopify" ? <><label className="key-label">Store domain<input value={values.storeDomain ?? ""} onChange={e => setValues(v => ({ ...v, storeDomain: e.target.value }))} placeholder="your-store.myshopify.com" /></label><label className="key-label">Admin access token<input type="password" value={values.accessToken ?? ""} onChange={e => setValues(v => ({ ...v, accessToken: e.target.value }))} placeholder="shpat_…" autoComplete="new-password" /></label>{action === "update_product_title" && <><label className="key-label">Product GID<input value={values.productId ?? ""} onChange={e => setValues(v => ({ ...v, productId: e.target.value }))} placeholder="gid://shopify/Product/…" /></label><label className="key-label">New title<input value={values.title ?? ""} onChange={e => setValues(v => ({ ...v, title: e.target.value }))} placeholder="Updated title" /></label></>}</> : <><label className="key-label">Slack bot token<input type="password" value={values.botToken ?? ""} onChange={e => setValues(v => ({ ...v, botToken: e.target.value }))} placeholder="xoxb-…" autoComplete="new-password" /></label>{action === "send_message" && <><label className="key-label">Channel ID<input value={channel} onChange={e => setChannel(e.target.value)} placeholder="C0123456789" /></label><label className="key-label">Message<input value={text} onChange={e => setText(e.target.value)} placeholder="Write the approved message" /></label></>}</>}<div className="provider-actions"><button className="primary-small" disabled={!user || save.isPending} onClick={saveConnection}>{save.isPending ? "Saving…" : "Save connection"}</button><button className="test-small" disabled={!user || preview.isPending} onClick={previewAction}>{preview.isPending ? "Preparing…" : "Preview action"}</button>{approvalId && <button className="primary-small" disabled={approve.isPending || execute.isPending} onClick={approveAndExecute}>{approve.isPending || execute.isPending ? "Executing…" : "Approve & execute"}</button>}</div></div></div></div>;
}
