...
describe("providerDb Persistent Store", () => {
  ...
  it("routes to custom provider when connected and requested, and falls back to Gemini 3.5 Flash on default", async () => {
    ...
    try {
      ...
      expect(defaultReq?.model).toBe("gemini-3.5-flash");
    } finally {
