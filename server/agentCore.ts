...
export class TaskSchedulerManager {
  ...
  listTasks(userId?: number): ScheduledTask[] {
    ...
    if (userId !== undefined) return all.filter((t) => t.userId === userId || !t.userId);
    return all;
  }
...
const defaultTools: AgentTool[] = [
  ...
  },
  {
    ...
    execute: async (args, context) => {
      ...
      const schedule = String(args.schedule || args.cron || "At specified time");
...
export class ToolExecutionEngine {
  ...
  async execute(toolId: string, arguments_: Record<string, unknown>, context: ToolExecutionContext, approved = false): Promise<NormalizedToolResult> {
    ...
    try {
      const result = await withTimeout(tool.execute(arguments_, context), tool.timeoutMs ?? 45_000);
...
export function buildAgentPlan(prompt: string): AgentPlan {
  ...
  if (/(schedule|task|reminder|cron|timer|later|recurring|at 5pm|at 10am|daily|weekly)/.test(lower)) {
    add("task.schedule", "Schedule the requested task or reminder with specified trigger parameters.");
  }
...
export function buildAgentTrace(plan: AgentPlan, providerError = false): AgentTrace[] {
  return [
    ...
    { stage: "analyze", status: "completed", detail: "Context, parameters, and workspace capabilities evaluated." },
    ...
    { stage: "tool_selection", status: "completed", detail: `${plan.tools.map(t => t.id).join(", ") || "none"} scoped for execution.` },
    ...
    { stage: "reflect", status: "completed", detail: "Output verified for correctness and safety." },
  ];
}
