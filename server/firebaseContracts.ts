 ...
 */
export function assertFirebaseBackend(
  backend: FirebaseBackend | undefined
): asserts backend is FirebaseBackend {
  if (!backend)
    throw new Error(
      "Firebase backend is not configured. Add the server-side Firebase adapter before enabling this operation."
    );
}
